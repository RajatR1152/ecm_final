...
export default function Mobiles() {
    ...
    function getProds() {
        axios.post(`${process.env.NEXT_PUBLIC_SERVER_URL}/mobiles`, { category: 'mobiles' }).then((res) => {
            setData(res.data);
        });
    ...
    return (
        <>
            <div className="container w-full flex flex-col md:flex-row items-center">
                <h1 className="text-4xl my-5 font-semibold capitalize ms-10">top deals on mobiles</h1>
            ...
            <div className="container md:columns-2 lg:columns-4 w-full p-5">
                {
                    data.map((d, i) => {
                        if (i < 8) return (
                            <Link onCanPlay={()=>{setLoading(true)}} href={`/product/${d._id}`} key={i} className="container mt-1 flex flex-col items-center justify-center border rounded-xl my-3 w-full p-3 h-80">
