...
export default function Tshirts({ u }) {
    ...
    function getProds() {
        axios.post(`${process.env.NEXT_PUBLIC_SERVER_URL}/tshirts`, { category: 'laptops' }).then((res) => {
            setData(res.data);
        });
    }
    ...
    return (
        <div >
            <div className="container w-full flex flex-col md:flex-row items-center">
                <h1 className="text-4xl my-5 font-semibold capitalize ms-10">{u == 1 ? 'top deals on clothings :' : 'similar products :'}</h1>
            ...
            <div className="container md:columns-2 lg:columns-4 w-full p-5">
                {
                    data.map((d, i) => {
                        if (i < 8) return (
                            <Link onClick={()=>{setLoading(true)}} href={`/product/${d._id}`} key={i} className="container mt-1 flex flex-col items-center justify-center border rounded-xl my-3 w-full p-3 h-72">
